...
$('#box').paging({
    ...
    callback: function(page) { // 回调函数
    	console.log(page);
    	$.ajax({
    		url:"<%=basePath%>/caidan/selectAll?page="+page,
    		method : "get",
    		success : function(data) {
    			...
    			for (var i = 0;i < data.length;i++) {
    				$("#tbody").append('<tr><td>'+data[i].mId+'</td>'
    						+'<td>'+data[i].mName+'</td>'
    						+'<td>'+data[i].pId+'</td>'
    						+'<td><a href="<%=basePath%>/caidan/xiugaiMenus?mId='+data[i].mId+'">点击修改</a></td>'
    						+'<td><a href="<%=basePath%>/caidan/deleteMenus?mId='+data[i].mId+'">点击删除</a></td>'
    						+'<td><a href="<%=basePath%>/caidan/addChildMenus?mId='+data[i].mId+'">点击添加子菜单</a></td>'
    						+'</tr>')
    			}
    		}
    	});
